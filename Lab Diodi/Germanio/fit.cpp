...
void IV(){
    ...
    pad2->SetGridy();
    pad2->SetLogy();
    TGraphErrors* grl = new TGraphErrors("data.dat","%lg %lg %lg %lg");
    grl->SetTitle("Caratteristica I-V del diodo al germanio fit lineare in scala semilogaritmica;ddp ai capi del Diodo [mV];Corrente nel diodo [mA]");
    grl->Draw("APE");
    leg->Draw("SAME");
    fl->Draw("SAME");
